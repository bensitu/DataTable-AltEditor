...
Merge the intended release code into the default branch with matching versions in `package.json` and `package-lock.json`. Add the release notes under the corresponding version heading in `CHANGELOG.md`. Both `## 4.0.2 - YYYY-MM-DD` and `## [4.0.2] - YYYY-MM-DD` heading formats are accepted.
