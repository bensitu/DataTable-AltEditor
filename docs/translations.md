...
Each supplied file contains all 24 string values from the AltEditor language defaults: `modalClose`, `edit.title`, `edit.button`, `add.title`, `add.button`, `delete.title`, `delete.button`, `deleteMessage`, `success`, `error.message`, `error.label`, `error.responseCode`, `error.required`, `error.unique`, `error.editSelection`, `error.deleteSelection`, `error.targetUnavailable`, `error.invalidResponse`, `error.invalidSetter`, `error.fileRead`, `error.fileAborted`, `error.fileSize`, `error.dialogFramework`, and `error.nativeDialog`.
