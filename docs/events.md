...
Native and custom field validation complete before pre-submit and submit events. Failed validation emits the existing error event without invoking persistence. Pending validation uses the same busy state and stale-result checks as persistence.
