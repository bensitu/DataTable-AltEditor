...
[Example 19](../example/19_native_dialog/example19.html) demonstrates explicit native mode with feature detection and no UI framework assets. The browser owns focus containment and background inertness. Close buttons cancel the dialog; Escape and backdrop dismissal remain disabled, matching the existing editor behavior. Successful saves follow `closeModalOnSuccess`. Pending saves disable close and submit buttons.

Native dialogs use the browser top layer. Third-party popup controls must support rendering their popup inside the dialog; a popup appended to `document.body` can appear behind it. Use Bootstrap / Foundation for integrations that cannot configure a popup container. Templates on those existing adapters preserve their current plugin integration behavior.
