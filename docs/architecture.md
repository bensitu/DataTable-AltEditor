...
| [Options](../src/core/options.js), [columns](../src/core/columns.js), [language](../src/core/language.js), and [events](../src/core/events.js) | Resolve settings and column metadata and emit the documented DataTables events.                                                                                                                                                     |
| [Data utilities](../src/data/)                                                                                                                 | Resolve row identity, update nested paths, normalize field values, and accept only the first callback or thenable persistence settlement, normalize errors, and run custom validators on isolated submission data.                  |
...
| [Dialog view](../src/dialog/dialog-view.js) and [field renderer](../src/dialog/field-renderer.js)                                              | Construct the dialog shell and framework presentation; build detached field controls without opening dialogs.                                                                                                                       |
| [Dialog forms](../src/dialog/dialog-form.js)                                                                                                   | Compose generated fields with templates and share add/edit opening, plugin initialization, and field population.                                                                                                                    |
...
| [Plugins](../src/dialog/plugins.js) and [framework adapters](../src/dialog/adapters/)                                                          | Initialize and release optional field controls and bridge Bootstrap, Foundation, or optional native dialog APIs. The [event bridge](../src/dialog/adapters/events.js) translates framework notifications into lifecycle operations. |
...
Field rendering does not depend on an editor instance or framework APIs. Template composition consumes detached fields directly without cloning them again; application templates are still cloned. Form coordination owns add/edit initialization, while the dialog controller owns operation state and failure cleanup. Failed field initialization closes the incomplete dialog, releases plugin resources, and reports an opening error.
...
Field feedback maps normalized errors to generated controls and plugin replacements, restores accessibility attributes when cleared, and falls back to global messages for unavailable fields. Dialog and inline editing share error normalization and the custom validator runner; framework adapters do not implement validation rules.

## Operation lifetime

Dialog submissions capture eligible field names before asynchronous collection and build their candidate before calling application persistence. Later changes to control values or disabled states do not alter an accepted submitted candidate. A persistence Promise can resolve an explicit replacement row when server processing changes values.
...
Dialog configuration selects an existing framework adapter or explicitly opts into native dialogs. Adapter selection does not add runtime dependencies. The [adapter registry](../src/dialog/adapters/index.js) resolves the requested framework independently of DOM presentation. The controller coordinates synchronous opening and render notifications and prevents recursive opening or replacement during closure. It captures selected identities before opening callbacks and resolves them again afterward, so callback-driven table changes cannot redirect editing or deletion. Bootstrap closing requests made during its show transition run after the shown event. Native mode delegates modal focus containment to the browser and retains the editor's save and cleanup contracts.
