 ...
 * @version 3.2
 * @file dataTables.altEditor.free.js
 * @author kingkode (www.kingkode.com)
 *  Modified by: Kasper Olesen (https://github.com/KasperOlesen), Luca Vercelli (https://github.com/luca-vercelli), Zack Hable (www.cobaltdevteam.com), Ben Situ (https://github.com/bensitu)
 * @contact www.kingkode.com/contact
 * @contact zack@cobaltdevteam.com
 * @copyright Copyright 2026 Kingkode
 *
        ...
        module.exports = function (root, $) {
            if (!root) {
                if (typeof window === 'undefined') {
                    throw new Error('altEditor requires a window object in CommonJS environments');
                }
                root = window;
            }
...
(function ($, window, document, undefined) {
    ...
    if (!$ || !$.fn || !$.fn.dataTable) {
        throw new Error('altEditor requires jQuery and DataTables');
    }
    ...
    $.extend(
        altEditor.prototype,
        {
            ...
            _setup: function () {
                ...
                var buttonActions = [
                    { name: 'edit', open: function () { that._openEditModal(); }, form: 'edit', submit: function () { that._editRowData(); } },
                    { name: 'delete', open: function () { that._openDeleteModal(); }, form: 'delete', submit: function () { that._deleteRow(); } },
                    { name: 'add', open: function () { that._openAddModal(); }, form: 'add', submit: function () { that._addRowData(); } }
                ];

                if (typeof dt.button === 'function') {
                    buttonActions.forEach(function (definition) {
                        var button = dt.button(definition.name + ':name');
                        if (!button || (typeof button.count === 'function' && button.count() === 0)) return;
                ...
                var getColumnNumberByName = function (name) {
                    ...
                    dt.columns().every(function (i) {
                        ...
                        var $header = headerNode && typeof headerNode.to$ === 'function' ? headerNode.to$() : $(headerNode);
                        if ($header.attr('name') === name) {
                            index = i;
                            return false;
                        }
                    });
                    if (index === null && dt.context && dt.context[0] && Array.isArray(dt.context[0].aoColumns)) {
                        var fallback = dt.context[0].aoColumns.findIndex(function (column) {
                            return (column.data !== undefined ? column.data : column.mData) === name;
                        });
                ...
                var checkUnique = function (event) {
                    ...
                    var editIndex = formName.indexOf('altEditor-edit-form-') === 0 && that._editSnapshot
                ...
                if (typeof dt.button === 'function') {
                    ...
                    if (refreshButton && (!refreshButton.count || refreshButton.count() > 0)) {
                        refreshButton.action(function (_event, tableApi) {
                            if (tableApi.ajax && typeof tableApi.ajax.url === 'function' && tableApi.ajax.url()) {
            ...
            _openEditModal: function () {
                ...
                columnDefs.forEach(function (columnDef) {
                    ...
                    var $element = $(that.modal_selector).find(selector).filter(':input[type!="file"]');
            ...
            _editRowData: function () {
                ...
                this._collectFormData($form).then(function (rowData) {
                    try {
                        that.onEditRow(
                            that,
                            rowData,
                            function (data) { that._editRowCallback(data); },
                            function (error) { that._errorCallback(error); },
            ...
            _openDeleteModal: function () {
                ...
                var fill = function () {
                    ...
                    $modal.find('.modal-body').empty().append(
                        $('<p/>', { 'class': 'altEditor-delete-message' }).text(that.language.deleteMessage)
                    );
            ...
            _openAddModal: function () {
                ...
                var that = this;
                columnDefs.forEach(function (columnDef) {
                    if (!columnDef.name || columnDef.value === null || columnDef.value === undefined) return;
                    var selector = '#' + String(columnDef.name).replace(/\./g, '\\.');
                    var $element = $(that.modal_selector).find(selector).filter(':input[type!="file"]');
                    if (!$element.length) return;
            ...
            selectionListener: function() {
                ...
                var toggleEditButton = function () {
                    ...
                    if (!buttons || (typeof buttons.count === 'function' && buttons.count() === 0)) return;
            ...
            */
            completeColumnDefs: function () {
                ...
                return dt.context[0].aoColumns.map(function (obj) {
                    return {
                        title: obj.sTitle,
                        placeholder: obj.placeholder !== undefined ? obj.placeholder : (obj.title !== undefined ? obj.title : obj.sTitle),
                        ...
                        datetimepicker: obj.datetimepicker !== undefined ? obj.datetimepicker : false,
                        editorOnChange: obj.editorOnChange !== undefined ? obj.editorOnChange : null,
                        style: obj.style !== undefined ? obj.style : '',
                        dateFormat: obj.dateFormat !== undefined ? obj.dateFormat : '',
                        optionsSortByLabel: obj.optionsSortByLabel !== undefined ? obj.optionsSortByLabel : false,
            ...
            */
            createDialog: function(columnDefs, modalTitle, buttonCaption, closeCaption, buttonClass, formName) {
                ...
                columnDefs.forEach(function (columnDef) {
                    var title = String(columnDef.title || '').replace(/(<([^>]+)>)/gi, '').trim();
                    ...
                    if (String(columnDef.type).indexOf('hidden') >= 0) {
                        ...
                        if (columnDef.value !== undefined && columnDef.value !== null) hidden.value = columnDef.value;
                    ...
                    formGroup.className = 'form-group row' + (columnDef.visible === false ? ' nonDisplay' : '');
                    formGroup.id = 'alteditor-row-' + String(columnDef.name);

                    if (!columnDef.inline || inlineCount === 0) {
                        var labelCol = document.createElement('div');
                        labelCol.className = 'col-12 col-sm-12 col-md-4 text-left text-sm-left text-md-right';
                    ...
                    inputCol.className = columnDef.inline ? 'col-sm-2 col-md-2 col-lg-2' : 'col-12 col-sm-12 col-md-8';
                    formGroup.appendChild(inputCol);

                    var type = String(columnDef.type || 'text');
                    if (type.indexOf('select') >= 0) {
                        var select = document.createElement('select');
                        select.className = 'w-100 form-control form-control-sm' + (columnDef.select2 ? ' select2' : '');
                        select.id = String(columnDef.name);
                        that._setElementAttributes(select, columnDef, ['name', 'style', 'disabled', 'required', 'multiple']);
                        select.setAttribute('data-unique', columnDef.unique ? 'true' : 'false');
                        if (columnDef.placeholder) select.setAttribute('data-placeholder', String(columnDef.placeholder));
                        ...
                        normalized.forEach(function (option) {
                            ...
                            if (String(option.value) === String(columnDef.value)) optionElement.selected = true;
                        ...
                        that._setElementAttributes(textarea, columnDef, ['name', 'style', 'rows', 'cols', 'maxLength', 'readonly', 'disabled', 'required']);
                        textarea.placeholder = String(columnDef.placeholder || title);
                        textarea.setAttribute('data-unique', columnDef.unique ? 'true' : 'false');
                        if (columnDef.value !== undefined && columnDef.value !== null) textarea.value = columnDef.value;
                        inputCol.appendChild(textarea);
                    }
                    else {
                        var input = document.createElement('input');
                        input.className = 'w-100 form-control form-control-sm' + (columnDef.readonly ? ' readonlyText' : '');
                        ...
                        that._setElementAttributes(input, columnDef, ['type', 'pattern', 'accept', 'name', 'step', 'min', 'max', 'maxLength', 'readonly', 'disabled', 'required', 'style']);
                        if (columnDef.value !== undefined && columnDef.value !== null) input.value = columnDef.value;
                ...
                var fill = function () {
                    ...
                    $modal.find('.modal-body').empty().append(that._currentDialogFragment.cloneNode(true));
                    $modal.find('.modal-footer').empty()
                        .append($('<button/>', {
                            type: 'button',
                            'class': 'btn btn-default button secondary',
                    ...
                    var modalContent = $modal.find('.modal-content');
                    if (modalContent.parent().is('form')) {
                        modalContent.parent().attr('name', formName).attr('id', formName).addClass('needs-validation');
                    } else {
                        modalContent.wrap($('<form/>', {
                            name: formName,
                            id: formName,
                    ...
                    Object.keys(error.responseJSON.errors).forEach(function (key) {
                        ...
                        if (Array.isArray(value)) {
                            value.forEach(function (item) {
                                if (item !== null && item !== undefined) messages.push(String(item));
                            });
            ...
            internalOpenDialog: function (selector, onopen) {
                try {
                    ...
                    if (typeof $sel.modal === 'function') {
                        $sel.off('show.bs.modal.altEditorFill')
                            .on('show.bs.modal.altEditorFill', onopen);
                        $sel.modal('show');
                    }
                    else if (typeof $sel.foundation === 'function' && window.Foundation && window.Foundation.Reveal) {
                        $sel.off('open.zf.reveal.altEditorFill')
                        ...
                        throw new Error('Bootstrap Modal or Foundation Reveal is required to open altEditor dialogs');
                    }
                } catch (error) {
            ...
            */
            ...
            _destroy: function () {
                ...
                if (this._languageRequest && typeof this._languageRequest.abort === 'function') {
                    try { this._languageRequest.abort(); } catch (_error) { }
                    this._languageRequest = null;
                }
                ...
                if (this.modal_selector) {
                    ...
                    if (reveal && typeof reveal.destroy === 'function') {
                        try { reveal.destroy(); } catch (_error) { }
                    }
                ...
                if (this.s && this.s.dt) {
                    ...
                    var body = this.s.dt.table && this.s.dt.table().body ? this.s.dt.table().body() : null;
            ...
            _initializePlugins: function () {
                ...
                this.columnDefs.forEach(function (columnDef) {
                    ...
                    if (String(columnDef.type || '').indexOf('select') >= 0 && columnDef.optionsSortByLabel) {
                    ...
                    if (columnDef.select2) {
                        if (typeof $.fn.select2 === 'function') {
                            var config = typeof columnDef.select2 === 'object' ? $.extend({}, columnDef.select2) : {};
                            config.dropdownParent = $(that.modal_selector);
                            $element.select2(config);
                        }
                    }
                    else if (columnDef.datepicker && typeof $.fn.datepicker === 'function') {
                        $element.datepicker(columnDef.datepicker);
                    }
                    else if (columnDef.datetimepicker && typeof $.fn.datetimepicker === 'function') {
                    ...
                    if (typeof columnDef.editorOnChange === 'function') {
                        ...
                        $element.off('change' + that.s.modalNamespace)
            ...
            _cleanupPlugins: function () {
                ...
                var selector = this.modal_selector;
                if (typeof $.fn.select2 === 'function') {
                    $(selector).find('select.select2-hidden-accessible').each(function () {
                        try { $(this).select2('destroy'); } catch (_error) { }
                    });
                }
                if (typeof $.fn.datepicker === 'function') {
                    $(selector).find('.hasDatepicker').each(function () {
                        try { $(this).datepicker('destroy'); } catch (_error) { }
                    });
                }
            ...
            _setDialogSubmitting: function (submitting) {
                ...
                $modal.find('button[data-dismiss="modal"], button[data-close]')
                    .prop('disabled', this._submitting);
            },
            ...
            _completeSuccessfulSubmit: function () {
                if (this.closeModalOnSuccess) {
                    ...
                    $(this.modal_selector).find('button[type="submit"]').prop('disabled', true);
                }
            },
            ...
            _showErrorMessage: function (message) {
                ...
                if (message) {
                    $('<br/>').appendTo($alert);
                    $('<span/>').css('white-space', 'pre-line').text(String(message)).appendTo($alert);
                }
                $body.append($alert);
            },
            ...
            _normalizeResponseData: function (response) {
                ...
                if (trimmed.charAt(0) === '{' || trimmed.charAt(0) === '[') {
                    try { return JSON.parse(trimmed); } catch (_error) { return response; }
                }
                return response;
            },
                ...
                if (Array.isArray(options)) {
                    return options.map(function (option) {
                        if (option && typeof option === 'object') {
                            var value = option.id !== undefined ? option.id : (option.value !== undefined ? option.value : '');
                            var label = option.text !== undefined ? option.text : (option.label !== undefined ? option.label : value);
            ...
            _setFieldValue: function ($element, columnDef, value) {
                ...
                if (type.indexOf('select') >= 0) {
                    ...
                    if (typeof normalized === 'string') {
                        ...
                        if (trimmed.charAt(0) === '[' || trimmed.charAt(0) === '{') {
                            try { selectValue = JSON.parse(trimmed); } catch (_error) { }
                        }
                    }
                ...
                if (type.indexOf('checkbox') >= 0) {
                    var checked = normalized === true || normalized === 1 || ['true', '1', 'yes', 'on'].indexOf(String(normalized).toLowerCase()) >= 0;
                    $element.prop('checked', checked);
                    return;
                }

                if (type.indexOf('date') >= 0 && columnDef.dateFormat && typeof window.moment === 'function') {
            ...
            _setValueByPath: function (target, path, value) {
                ...
                if (!segments.length || segments.some(function (segment) { return !segment || blocked[segment]; })) {
            ...
            _collectFormData: function ($form) {
                ...
                $form.find('select, textarea, input').each(function () {
                    ...
                    if (type === 'file') {
                        ...
                        if (!file) return;
                        if (that.encodeFiles) {
                            fileTasks.push(new Promise(function (resolve, reject) {
                                that.getBase64(file, function (content) {
                                    try { that._setValueByPath(values, id, content); resolve(); }
                ...
                return Promise.all(fileTasks).then(function () { return values; });
            },
            ...
            _resolveSnapshotRow: function (snapshot) {
                ...
                if (snapshot.rowNode && snapshot.rowNode.isConnected && snapshot.rowNode.closest('table') === table) {
